...
function isAbsolutePath(value) {
	return (
		path.resolve(value) ===
		path.normalize(value).replace(new RegExp(path.sep + "$"), "")
	);
}
	...
	switch (key) {
		...
		case "email rules":
			throw new Error(
				'The option "' +
					key +
					'" is no longer supported. See https://github.com/safeh2o/keystone-v4/wiki/0.3.x-to-0.4.x-Changes'
			);
		...
		case "mongo":
			if (typeof value !== "string") {
				if (
					...
					value =
						value.length === 2
							? "mongodb://" + value[0] + "/" + value[1]
							: "mongodb://" + value[0] + ":" + value[2] + "/" + value[1];
				} else {
					console.error(
...
exports.expandPath = function (pathValue) {
	pathValue =
		...
		pathValue.substr(0, 1) !== path.sep &&
		pathValue.substr(1, 2) !== ":\\"
